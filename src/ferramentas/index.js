...
const getUsuarioByEmail = async (email) => {
    const { rowCount, rows } = await conexao.query('SELECT * FROM base_users WHERE email = $1', [email]);
...
const getUsuarioById = async (id) => {
    const { rowCount, rows } =
        await conexao.query('SELECT * FROM base_users WHERE id = $1', [id]);
    const userEncontrado = rowCount > 0;
...
const isNotUnicEmail = async (email) => {
    const { rowCount } =
        await conexao.query('SELECT email FROM base_users WHERE email = $1', [email]);
    return rowCount > 0;
}
